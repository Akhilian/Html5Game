function Camera(map, character) {
 ...
 Camera.prototype.draw = function() {
	...
	if ( viewport.x + viewport.width > ( this.map.world.width * TILE_SIZE ) )
		viewport.x = ( this.map.world.width * TILE_SIZE ) - viewport.width;
	...
	for(var i = 0; i < this.map.mapRaw.field.length; i++) {
		...
		var item = this.map.mapRaw.field[i],
			...
			imgTile.drawAt((item.x * TILE_SIZE) - viewport.x, (item.y * TILE_SIZE) - viewport.y, true);
	}
